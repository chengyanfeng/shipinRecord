...
public class RecordingSDK {
  ...
  /** This method creates a channel and enables the recording application to join with the user account.
   * 
   * After the recording application successfully joins the channel, the SDK triggers the {@link RecordingEventHandler.onLocalUserRegistered onLocalUserRegistered} and {@link RecordingEventHandler.onJoinChannelSuccess onJoinChannelSuccess} callbacks on the local client.
   * 
   * @note To ensure smooth communication, use the same parameter type to identify the users in the same channel. Hence, the parameter type of the recording application's identifier should be the same as that of the other users joining the channel with the Agora Native/Web SDK.
   * 
   * @param appId Set `appId` of the recording application the same as that of the Native/Web SDK. A channel does not accept duplicate uids. Otherwise, there will be unpredictable behaviors. For more information, see <a href="https://docs.agora.io/en/Recording/token?platform=All%20Platforms">Getting an App ID</a>. 
   * 
   * @param channelKey The dynamic key for authentication. Set `channelKey` of the recording application the same as that of the Native/Web SDK. If the Native/Web SDK uses a token, `channelKey` must be set as the token. For more information, see <a href="https://docs.agora.io/en/Recording/token?platform=All%20Platforms">Use Security Keys</a>. In the Recording SDK, `requestToken` and `renewToken` are private interfaces. Therefore, ensure that you set <a href="https://docs.agora.io/en/Recording/token?platform=All%20Platforms#Generate_Token">expireTimestamp</a> as 0 when generating a token, which means that the privilege, once generated, never expires.
   * 
   * @param name The name of the channel to be recorded.
   * 
   * @param userAccount The user account of the recording application. The maximum length of this parameter is 255 bytes. Ensure that you set this parameter and do not set it as null. Supported character scopes are:
   * <ul>
